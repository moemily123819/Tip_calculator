// Author : Emily Mo
// Date : Jun 25, 2019
// Javascript : to receive 3 variables from index.html for the calculation of tip 
// the three variables are : bill amount, tip percentage and number of persons to split the bill
//
//


// subliminal messages - to recommend a fair tip percentage to the server
...
function msg_change(element) {
 ...
 // to test if the input is a floating number    
...
// reset - remove replies from previous input, clear bill amounts and reset tip and number of patrons
...
// calculate total bill amount = bil amount * (1 + tip percentage) / 100  
// calculate tip amount = bill amouint * tip percentage / 100
// if more than 1 person to pay the bill : 
//      bill per person =total bill amount / number of persons
//      tip per person = total tip amount / number of persons
//
// the tricky part of this program is rounding of various numbers in the process of calculating total bill amount, 
// bill amount paid by each patron, or tip amount paid by each patron
// 
// to conquer the tricky part, the total amount is first calculated..rounding to the nearest 2 decimal places
// then this amount is to be split among patrons....so that not a penny lower than the total bill amount
// same goes to the tip to be paid by each person.....rounding is needed and no penny left unpaid !!!!!
